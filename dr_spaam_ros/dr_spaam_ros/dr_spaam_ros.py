...
class DrSpaamROS(Node):
    ...
    def _init(self):
        ...
        """
        # Publisher
        self._dets_pub = self.create_publisher(PoseArray,
            "/dr_spaam_ros_detections", 1
        )

        self._rviz_pub = self.create_publisher(Marker,
            "/dr_spaam_ros_rviz", 1 )

        # Subscriber
        self._scan_sub = self.create_subscription(
            LaserScan, "/scan", self._scan_callback, 1
        )
    ...
    def _scan_callback(self, msg):
        ...
        # print("xy coord:{}".format(dets_xy))
        print("estimate person:{}".format(dets_xy.shape[0]))
